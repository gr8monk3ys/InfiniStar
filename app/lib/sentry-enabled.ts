...
// `sentry.server.config.ts`, `sentry.edge.config.ts`) so the four of them
// cannot drift apart.
//
...
// `environment:development` — laptops running an app locally with a DSN in
...
//
// NEXT_PUBLIC_VERCEL_ENV is what the browser bundle can read —
// `next.config.mjs` defines it from VERCEL_ENV at build time so it is present
// whether or not the Vercel project exposes system variables to the client.
//
...
/**
 * The value events are tagged with, so production and preview stay tellable
 * apart (these configs tagged no environment at all before). An explicit
 * SENTRY_ENVIRONMENT, which env.mjs already declares, wins over the inferred
 * one.
 */
export const SENTRY_ENV = process.env.SENTRY_ENVIRONMENT || VERCEL_ENV || "development"
